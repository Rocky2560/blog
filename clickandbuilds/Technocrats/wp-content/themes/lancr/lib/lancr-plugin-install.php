...
if ( ! class_exists( 'Lancr_Plugin_Install' ) ) :
	/**
	 * The Lancr plugin install class
	 */
	class Lancr_Plugin_Install {
		...
		public static function install_plugin_button( $plugin_slug, $plugin_file, $plugin_name, $classes = array(), $activated = '', $activate = '', $install = '' ) {
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						'message' => esc_attr__( 'Activated', 'lancr' ),
						'url'     => '#',
						'classes' => array( 'lancr-button', 'disabled' ),
					);
					...
					$button = array(
						'message' => esc_attr__( 'Activate', 'lancr' ),
						'url'     => $url,
						'classes' => array( 'lancr-button', 'activate-now' ),
					);
					...
					$button = array(
						'message' => esc_attr__( 'Install now', 'lancr' ),
						'url'     => $url,
						'classes' => array( 'lancr-button', 'lancr-install-now', 'install-now', 'install-' . $plugin_slug ),
					);
				...
				?>
				<span class="lancr-plugin-card plugin-card-<?php echo esc_attr( $plugin_slug ); ?>">
				...
				<a href="https://wordpress.org/plugins/<?php echo esc_attr( $plugin_slug ); ?>" target="_blank"><?php esc_html_e( 'Learn more', 'lancr' ); ?></a>
				<?php
			}
		}
...
return new Lancr_Plugin_Install();
