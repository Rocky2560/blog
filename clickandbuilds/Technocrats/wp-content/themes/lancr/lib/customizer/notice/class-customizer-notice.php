<?php
/**
 * Lancr Customizer Notification System.
 *
 * @package Lancr
 */
...
/**
 * Lancr Customizer Notify Class
 */
class Lancr_Customizer_Notice {
	...
	/**
	 * The single instance of Lancr_Customizer_Notice
	 *
	 * @var Lancr_Customizer_Notice $instance The Lancr_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Lancr_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Lancr_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Lancr_Customizer_Notice ) ) {
			self::$instance = new Lancr_Customizer_Notice;
	...
	public function setup_config() {
		...
		$lancr_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$lancr_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'lancr-customizer-notice', get_template_directory_uri() . '/lib/customizer/notice/css/customizer-notice.css', array() );
		...
		wp_enqueue_script( 'lancr-customizer-notice', get_template_directory_uri() . '/lib/customizer/notice/js/customizer-notice.js', array( 'customize-controls' ) );
		wp_localize_script(
			'lancr-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => esc_html__( 'Activating', 'lancr' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Lancr_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Lancr_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Lancr_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'lancr_show_recommended_plugins', $show_recommended_plugins );
		}
