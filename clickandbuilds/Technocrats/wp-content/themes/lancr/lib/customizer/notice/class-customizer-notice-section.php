<?php
/**
 * Lancr Customizer Notification Section Class.
 *
 * @package Lancr
 */

/**
 * Lancr_Customizer_Notice_Section class
 */
class Lancr_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		$json = parent::json();
		global $lancr_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'lancr_show_recommended_plugins' );

		foreach ( $lancr_recommended_plugins as $slug => $plugin_opt ) {
			...
			$lancr_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$lancr_recommended_plugin['class'] = 'active';
			} else {
				$lancr_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$lancr_recommended_plugin['button_class'] = 'install-now button';
					$lancr_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$lancr_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$lancr_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$lancr_recommended_plugin['button_class'] = 'deactivate-now button';
					$lancr_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$lancr_recommended_plugin['id'] = $slug;
			$lancr_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$lancr_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$lancr_recommended_plugin['description'] = $info->short_description;
			}

			$lancr_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $lancr_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="lancr-recommeded-actions-container lancr-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="lancr-recommeded-actions">
